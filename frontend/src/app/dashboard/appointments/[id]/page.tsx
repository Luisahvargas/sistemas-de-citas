...
export default function EditAppointmentPage() {
    ...
    const fetchAppointment = async () => {
      try {
        ...
        setForm({
          ...
          status:       a.status,
          delivered_at: a.delivered_at ? a.delivered_at.slice(0, 16) : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Error al actualizar la cita. Revisa los campos.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-800">Editar Cita</h1>
      </div>
      ...
      <div className="bg-white rounded-xl border p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {fieldErrors.status && (
              <p className="text-red-500 text-xs mt-1">{fieldErrors.status}</p>
            )}
          </div>
